...
export const ImagePacksPicker: React.FC<IProps> = ({ room, onFinished }) => {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            {/* Tab Bar */}
            <div style={{ display: 'flex', overflowX: 'auto', borderBottom: '1px solid var(--cpd-color-border-interactive-secondary)', padding: '8px' }}>
                {packs.map(p => {
                    const avatarUrl = p.pack.pack?.avatar_url;
                    ...
                    return (
                        <AccessibleButton
                            ...
                            style={{
                                padding: '4px',
                                borderBottom: activePackId === p.id ? '2px solid var(--cpd-color-icon-accent-tertiary)' : '2px solid transparent',
                                cursor: 'pointer',
                        ...
                        >
                            {src ? <img src={src} style={{ width: 32, height: 32, objectFit: 'contain' }} alt="" /> : <span style={{ width: 32, height: 32, display: 'inline-block', textAlign: 'center', lineHeight: '32px' }}>📁</span>}
                        </AccessibleButton>
                    );
                })}
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '12px' }}>
                {packs.length === 0 ? (
                    <p style={{ textAlign: 'center', color: 'var(--cpd-color-text-secondary)' }}>
                        {"Stickers & Emotes"}
                    </p>
                ) : (
                    packs.filter(p => p.id === activePackId).map(p => (
                        <div key={p.id}>
                            <h4 style={{ margin: '0 0 12px 0', fontSize: '14px' }}>{p.pack.pack?.display_name || p.id}</h4>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(64px, 1fr))', gap: '8px' }}>
                                {Object.entries(p.pack.images).map(([shortcode, image]) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            {src ? <img src={src} style={{ width: '100%', aspectRatio: '1', objectFit: 'contain' }} alt={image.body || shortcode} /> : <span style={{display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', fontSize: '10px'}}>{shortcode}</span>}
                                        </AccessibleButton>
                                    );
                                })}
                            </div>
